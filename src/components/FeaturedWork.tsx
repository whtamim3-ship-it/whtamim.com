    ...
    <SectionReveal id="work" className="py-24 w-full bg-[#F5F5F7]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
        {featuredProjects.map((project, idx) => (
          <TextReveal key={project.id} delay={0.08 * idx} yOffset={24}>
            <FeaturedProjectCard
              project={project}
              onSelect={() => {
      ...
      <TextReveal delay={0.2} yOffset={16} className="flex justify-end pt-4">
        <button
          ...
          className="group inline-flex items-center gap-2 text-14px font-medium text-[#1D1D1F] hover:text-[#007AFF] transition-colors py-2 px-4 rounded-full hover:bg-white/60"
        >
...
const FeaturedProjectCard: React.FC<FeaturedProjectCardProps> = ({ project, onSelect }) => {
  ...
  return (
    <div
      ...
      className="group cursor-pointer relative rounded-[24px] overflow-hidden bg-white border border-neutral-200/80 shadow-xs hover:shadow-2xl hover:border-neutral-300 transition-all duration-300 flex flex-col justify-between"
      style={{
    ...
    >
      ...
      <div
        ...
        style={{
          background: `radial-gradient(600px circle at ${tilt.glossX}% ${tilt.glossY}%, rgba(255,255,255,0.25), transparent 40%)`,
        }}
      />
      ...
      <div className="p-5 sm:p-7 flex flex-col justify-between flex-grow">
        <div>
          ...
          <p className="text-13px sm:text-14px text-[#86868B] line-clamp-2 leading-relaxed mb-4">
            {project.logline}
          </p>
        </div>
